...
def illuDecomp(inputs, am, nm, shadow, gamma, masks):
    ...
    inputs = tf.pow(inputs, gamma) * masks
    ...
    def body(i, output):
        ...
        inputs_pixels = tf.reshape(inputs_, (-1, 3))
        D_ = tf.reshape(D_, (-1, 3))
        ...
        output_r = tf.matmul(pinv(A * D_[:, 0:1]), inputs_pixels[:, 0:1])
        output_g = tf.matmul(pinv(A * D_[:, 1:2]), inputs_pixels[:, 1:2])
        output_b = tf.matmul(pinv(A * D_[:, 2:3]), inputs_pixels[:, 2:3])

        output = output.write(i, tf.concat([output_r, output_g, output_b], axis=-1))
        i += tf.constant(1)
