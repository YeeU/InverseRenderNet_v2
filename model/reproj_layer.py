...
def map_reproj(dm1, map2, cam1, cam2, scale_x1, scale_x2, scale_y1, scale_y2):
    ...
    vm, um = tf.meshgrid(tf.range(1.0, v1 + 1.0), tf.range(1.0, u1 + 1.0))
    ...
    pc = tf.stack(
        [
            tf.contrib.layers.flatten(Xm),
            tf.contrib.layers.flatten(Ym),
            tf.contrib.layers.flatten(dm1),
        ],
        axis=1,
    )
    ...
    homo_pc_c1 = tf.concat(
        [pc, tf.ones((batch_size, 1, num_pix), dtype=tf.float32)], axis=1
    )

    # both transformation matrix have shape batch-by-4-by-4, valid for multiplication with defined homogeneous point cloud
    last_row = tf.tile(
        tf.constant([[[0, 0, 0, 1]]], dtype=tf.float32), multiples=[batch_size, 1, 1]
    )
    ...
    homo_pc_c2 = tf.matmul(
        W_C_trans2, tf.matmul(tf.matrix_inverse(W_C_trans1), homo_pc_c1)
    )
    ...
    k2 = tf.stack(
        [
            tf.stack([f2[:, 0], zeros, c2[:, 0], zeros], axis=1),
            tf.stack([zeros, f2[:, 1], c2[:, 1], zeros], axis=1),
    ...
    u_valid = tf.logical_and(
        tf.logical_and(tf.logical_not(tf.is_nan(u_reproj)), u_reproj > 0),
        u_reproj < u2 - 1,
    )
    v_valid = tf.logical_and(
        tf.logical_and(tf.logical_not(tf.is_nan(v_reproj)), v_reproj > 0),
        v_reproj < v2 - 1,
    )
    ...
    img_inds = tf.tile(
        tf.expand_dims(tf.to_float(tf.range(batch_size)), axis=1),
        multiples=[1, num_pix],
    )
    request_points1 = tf.stack(
        [
            tf.boolean_mask(img_inds, pixels),
            tf.boolean_mask(v_reproj, pixels),
            tf.boolean_mask(u_reproj, pixels),
        ],
        axis=1,
    )
    ...
    shape = [batch_size, tf.to_int32(u1), tf.to_int32(v1), ndims]

    pixels = tf.reshape(
        pixels, shape=tf.stack([batch_size, tf.to_int32(u1), tf.to_int32(v1)], axis=0)
    )
...
def interpImg(unknown, data):
    ...
    avg_low_y = dist_low_x * tf.gather_nd(
        data, indices=tf.stack([img_inds, low_y, low_x], axis=1)
    ) + dist_high_x * tf.gather_nd(
        data, indices=tf.stack([img_inds, low_y, high_x], axis=1)
    )
    avg_high_y = dist_low_x * tf.gather_nd(
        data, indices=tf.stack([img_inds, high_y, low_x], axis=1)
    ) + dist_high_x * tf.gather_nd(
        data, indices=tf.stack([img_inds, high_y, high_x], axis=1)
    )
